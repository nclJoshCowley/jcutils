...
knit_print.list <- jcutils::knit_tabset
```
...
To affect all lists within a document, one can define a `list` only method for
    the generic `knit_print` which is explained [here][knit_print_vignette].
...
````markdown
```{r}`r ''`
#| render: !expr jcutils::knit_tabset
myList
```
````
...
```{r plotlist-tabs-nested}
geom_args <- list(both = c("line", "point"), line = "line", point = "point")

lapply(geom_args, function(.g) lapply(vars, gg_mtcars, geom = .g))
```
